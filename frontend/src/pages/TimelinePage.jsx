    ...
    if (loading) {
        return (
            <div className="min-h-screen bg-[#F8FAFC] flex flex-col items-center justify-center space-y-4">
                ...
                <p className="text-base font-medium text-slate-600">Loading chronological clinical audit...</p>
            </div>
        );
    }
            ...
            <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
                ...
                <div className="bg-white p-4 rounded border border-slate-200 shadow-xs flex flex-col sm:flex-row gap-4 items-center justify-between">
                    <div className="w-full sm:w-auto">
                        <input
                            ...
                            className="border border-slate-300 rounded px-3.5 py-2 text-sm bg-white focus:outline-none focus:ring-1 focus:ring-[#1C355E] focus:border-[#1C355E] w-full sm:w-80"
                        />
                    </div>

                    {/* Category Navigation Filter */}
                    <div className="flex items-center gap-2 overflow-x-auto pb-1 text-sm self-start sm:self-auto">
                        <button
                            onClick={() => setSelectedCategory('ALL')}
                            className={`px-3.5 py-2 font-semibold transition whitespace-nowrap cursor-pointer rounded border ${
                ...
                {filteredItems.length === 0 ? (
                    <div className="bg-white border border-slate-200 rounded p-12 text-center text-slate-500 space-y-3">
                        <div className="font-semibold text-slate-800 text-lg">No clinical records found in this view</div>
                        <p className="text-sm text-slate-500 max-w-sm mx-auto">
                            Upload documents across any of the 4 clinical categories to establish your chronological record.
                        </p>
                        <Link
                            to="/patient/dashboard"
                            className="inline-block px-5 py-2.5 bg-[#1C355E] hover:bg-[#15294A] text-white font-medium text-sm rounded transition mt-2"
                        >
                            Return to Dashboard
                        </Link>
                    </div>
                        ...
                        {filteredItems.map((item, index) => {
                            ...
                            return (
                                <div key={index} className="relative pl-6 sm:pl-8">
                                    {/* Timeline Marker */}
                                    <span className="absolute -left-[9px] top-2 bg-[#1C355E] text-white rounded-full h-4.5 w-4.5 flex items-center justify-center text-[10px] ring-4 ring-slate-100">
                                    </span>

                                    <div className="bg-white p-5 sm:p-6 rounded border border-slate-200 shadow-2xs space-y-3">
                                        ...
                                        <div className="text-sm sm:text-base text-slate-700 leading-relaxed bg-slate-50 p-4 rounded border border-slate-200">
                                            {item.description}
                                        </div>

                                        <div className="flex items-center justify-between pt-1 text-xs sm:text-sm">
                                            <span className="bg-slate-100 text-slate-700 border border-slate-300 px-2.5 py-1 rounded font-medium">
                                                {cat.name}
                                            </span>
                                            <Link
                                                to="/patient/dashboard"
